...
# 30 training pairs (AB, BC, CD, DE) + 10BD + 10AE.  
...
my_sampler = Sample_without_replacement(['t','nt', 'f1', 'f2', 'f3'])
familiar_sampler = Sample_without_replacement(['a','b', 'c', 'd'])
...
def init_trial(event):
    ...
    if event == "entry":
        ...
        next_familiar = familiar_sampler.next()
        if next_sample == "t":
            timed_goto_state("choice_state_t", v.ini_duration[0])
        elif next_sample == "nt":
            timed_goto_state("choice_state_nt", v.ini_duration[0])
        else:
            if next_familiar == "a":
                timed_goto_state("choice_state_a", v.ini_duration[0])
            elif next_familiar == "b":
                timed_goto_state("choice_state_b", v.ini_duration[0])
            elif next_familiar == "c":
                timed_goto_state("choice_state_c", v.ini_duration[0])
            elif next_familiar == "d":
                timed_goto_state("choice_state_d", v.ini_duration[0])
        
def choice_state_t(event):  # B vs D
    if event == "entry":
        hw.P_B.LED.on()
        hw.P_D.LED.on()
        print("Choice B vs D")
...
def choice_state_nt(event):  # A vs E
    if event == "entry":
        hw.P_A.LED.on()
        hw.P_E.LED.on()
        print("Choice A vs E")
...
def choice_state_d(event):  # D vs E
    if event == "entry":
        hw.P_D.LED.on()
        hw.P_E.LED.on()
        print("Choice D vs E")
...
def correct_sound(event):
    if event == "entry":
        if v.choice == "A":
            ...
            timed_goto_state("reward_d", v.audio_durations[0])
...
def reward_d(event): # Deliver reward to Port D.
    if event == "entry":
        timed_goto_state("inter_trial_interval", v.reward_durations[0])
        hw.P_D.SOL.on()
        v.n_rewards += 1
